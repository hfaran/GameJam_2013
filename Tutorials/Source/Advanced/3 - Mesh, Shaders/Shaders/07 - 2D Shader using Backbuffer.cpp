...
void CompileShaders()
{
   ...
   {
      ShaderCompile("Shader/2D Shader Invert.cpp", S+DataPath()+"Shader/GL/User/Custom Shader", SM_GL);
   }else
   if(D.shaderModel()>=SM_4)
   {
      ShaderCompile("Shader/2D Shader Invert.cpp", S+DataPath()+"Shader/4/User/Custom Shader", SM_4);
   }else
   {
      ShaderCompile("Shader/2D Shader Invert.cpp", S+DataPath()+"Shader/2/User/Custom Shader", SM_2);
      ShaderCompile("Shader/2D Shader Invert.cpp", S+DataPath()+"Shader/3/User/Custom Shader", SM_3);
   }
}
/******************************************************************************/
void InitPre()
{
   App.name("2D Shader using Backbuffer");
...
void Draw()
{
   ...
   C Image &image=Renderer.getBackBuffer(); // access current screen

   Shaders("User/Custom Shader")->getTech("CustomTechnique")->draw(image, &Rect_C(0, 0, 1, 1));
}
/******************************************************************************/
