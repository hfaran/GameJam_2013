...
Memb<PakFileData>        update_files; // list of files to be updated in our paks
...
Bool Update()
{
   ...
   if(Kb.bp(KB_SPACE))OSLaunch(paks_dir); // preview data folder when space pressed
   ...
   if(patcher.getNextDownload(downloaded)) // if there is a file downloaded
   {
      if(downloaded.success)
      {
         ...
         pfd.crc32          =downloaded.crc32;
         pfd.modify_time_utc=downloaded.modify_time_utc;
         Swap(pfd.file, downloaded.data); // swap the file data from 'downloaded' object to 'PakFileData'
      }else
      {
   ...
   }else
   {
      ...
      &&  compared           ) // the index was downloaded and we have performed the comparison
      {
         ...
         if(update_files.elms()) // if there are any files that need to be updated
         {
            ...
            PakUpdate(pak, update_files, pak_name, NULL); // perform the update
