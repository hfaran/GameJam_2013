...
void SQLTests()
{
   ...
   {
      if(!sql.connectMSSQL("LocalHost\\SQLExpress", "test_db", "", "", &messages))Exit(S+"Can't connect to Server!\nPlease read instructions in the tutorial codes.\nGiven Error:\n"+messages);
   }
   ...
   // create a sample table
   {
      if(!sql.existsTable(table_name)) // if it wasn't yet created
      {
         ...
         columns.New().set("registered", SDT_DATE_TIME).default_val="GetDate()";      // set a new column named "registered" of DATE_TIME type, and set its default value to a SQL function named "GetDate()"
   ...
   // add a player
   {
      if(!sql.newRow(table_name, SQLValues().New("name", "PlayerName").New("e_mail", "address@domain.com"), &messages))Exit(S+"Can't create player:\n"+messages);
      if(!sql.newRow(table_name, SQLValues().New("name", "Player #2" ).New("e_mail",     "xxx@domain.com"), &messages))Exit(S+"Can't create player:\n"+messages);
   }
...
void Draw()
{
   ...
   D.text(0, 0.9f, "Players:");
   FREPA(players)D.text(0, 0.7f-i*0.1f, players[i]);
}
/******************************************************************************/
